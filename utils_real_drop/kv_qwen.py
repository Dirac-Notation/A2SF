"""KV-compression-enabled Qwen2 (mirrors kv_llama.py).

Qwen2 is architecturally almost identical to Llama (RMSNorm, RoPE, GQA, SwiGLU
MLP, shared rotary position_embeddings). The only differences vs kv_llama:
  - q/k/v projections use bias=True, o_proj bias=False (Qwen2 hardcodes this).
  - no `pretraining_tp` o_proj split.
...
class Qwen2Attention(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        # Qwen2: q/k/v have bias, o_proj does not.
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        # No-padding fast path: skip 4D mask materialization (causal handled internally).
...
class KVQwen2ForCausalLM(Qwen2ForCausalLM):
