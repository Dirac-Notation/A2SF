...
class AverageCache(KVCache):
    ...
    def flash_prepare_scores(self, attn_scores):
        """Prepare scores for flash attention"""
        # Calculate current score for flash attention
        current_score = attn_scores.sum(self.seq_dim)
        current_count = (attn_scores > 0).sum(self.seq_dim).to(current_score.dtype)
        
        if self.score is None:
            return current_score, current_count
        else:
            temp_score = current_score.clone()
            temp_count = current_count.clone()
            temp_score[:,:,:-1] += self.score * self.cumulative_count
            temp_count[:,:,:-1] += self.cumulative_count
            return temp_score, temp_count
    ...
    def flash_attention(self, query, key, value, attn_mask, head_dim, block_size=1024):
        ...
        acc_count = torch.zeros((batch_size, num_heads, seq_len_k), dtype=query.dtype, device=query.device)
        ...
        for k_start in range(0, seq_len_k, block_size):
            ...
            temp_score, temp_count = self.flash_prepare_scores(scores)
            acc_score[:,:,k_start:k_end] += temp_score/temp_count
            acc_count[:,:,k_start:k_end] += temp_count
        ...
        if self.score is None:
            ...
            self.cumulative_count = acc_count
        else:
            acc_score[:,:,:-1] += self.score
            acc_count[:,:,:-1] += self.cumulative_count
            self.score = acc_score
            self.cumulative_count = acc_count
