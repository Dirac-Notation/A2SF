"""KV-compression-enabled Gemma (mirrors kv_llama.py).

Gemma vs Llama differences handled here:
  - head_dim is a separate config field (config.head_dim), so num_heads*head_dim
    need NOT equal hidden_size; o_proj maps (num_heads*head_dim) -> hidden_size.
  - token embeddings are scaled by sqrt(hidden_size) at the model input.
  - RoPE is computed inside each attention (no shared position_embeddings).
  - GemmaRMSNorm uses (1 + weight) in float32, GemmaMLP uses GeGLU.
The cache/attention/scorer/selector plumbing is identical and model-agnostic.
attention scaling 1/sqrt(head_dim) is applied inside compressed_attention via
head_dim, matching Gemma's self.scaling.
"""
...
class GemmaAttention(nn.Module):
    def __init__(self, config: GemmaConfig, layer_idx: Optional[int] = None):
        ...
        self.head_dim = config.head_dim            # NOTE: not hidden_size // num_heads
    ...
    def forward(
        ...
        **kwargs,
        ...
        # NOTE: reshape to num_heads*head_dim (may differ from hidden_size), then o_proj.
...
class GemmaModel(GemmaPreTrainedModel):
    def __init__(self, config: GemmaConfig):
        ...
        self.layers = nn.ModuleList([GemmaDecoderLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class KVGemmaForCausalLM(GemmaForCausalLM):
    ...
    def _prepare_cache_for_generation(
        ...
        model_kwargs["past_key_values"] = CompressedKVCache(
            config=self.model.config, compression_config=self.compression_config, device=inferred_device,
        )
