...
def make_hook(layer_idx, attn_module, op_type):
    ...
    def hook_fn(module, inputs, output):
        ...
        if op_type == "v":
            ...
            keies[layer_idx] = tensors.detach().to("cpu")
            return
...
input_ids = torch.cat([input_ids[:,:1024], input_ids[:,-1024:]], dim=1)
...
for layer_idx in tqdm(range(num_hidden_layers)):
    ...
    diff = (torch.matmul(query,key.transpose(1,2)) - torch.matmul(query,reconstructed_key.transpose(1,2))).abs()
    ...
    for head_idx in range(num_key_value_heads):
        ...
        # plot_matrix(skewed_key[head_idx], os.path.join(save_dir, f"key_{head_idx}.png"))
        # plot_matrix(reconstructed_key[head_idx], os.path.join(save_dir, f"key_{head_idx}.png"))
        
        for head_idx_inner in range(num_key_value_groups):
            plot_matrix(diff[head_idx_inner], os.path.join(save_dir, f"diff_{head_idx_inner}.png"))