...
- `kv_qwen.py` / `kv_opt.py` — same integration for Qwen2 / OPT.
  Qwen2 ≈ Llama (qkv bias=True). OPT: no RoPE (learned positions), no GQA, LayerNorm,
...
### Supported Models
LLaMA 3.2 1B / 3.1 8B Instruct, Qwen 2.5, OPT (`config/model2path.json`).
Architecture support via `kv_{llama,qwen,opt}.py`, dispatched by `config.model_type`.
