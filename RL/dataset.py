...
# The 13 paired (a, b) actions — single source of truth is RL/action_grid.py.
...
def _format_prompt(prompt: str, dataset_name: str, model_name: str) -> str:
    if str(dataset_name or "").strip().lower() not in ["trec", "triviaqa", "samsum", "lsht", "lcc", "repobench-p",
            "codeparrot_code", "csn_code", "ghcode", "agnews_fs","div_class","div_qa","div_dialog"]:  # my completion-style training sets (match LB no-chat)
...
def _build_compression_config(a_vals: torch.Tensor, b_vals: torch.Tensor, budget: int,
                              ada_kv: bool = False, curve: str = "sigmoid") -> CompressionConfig:
...
def _worker(
    ...
    a_list = [a for a, _ in actions] if actions else list(SIGMOID_A_VALUES)
    b_list = [b for _, b in actions] if actions else list(SIGMOID_B_VALUES)
    a_tensor = torch.tensor(a_list, dtype=torch.float32, device=model.device)
    b_tensor = torch.tensor(b_list, dtype=torch.float32, device=model.device)
    num_actions = len(a_list)
    ...
    try:
        while True:
            ...
            # LongBench-style long-context truncation (keep the END = query/last example) so the
            # 13-action prefill fits memory on 7B models; only the longest prompts are affected.
            _maxin = int(os.environ.get("MAX_INPUT_TOKENS", "0") or "0")
            if _maxin and input_ids.shape[-1] > _maxin:
                input_ids = input_ids[:, -_maxin:]
            input_ids = input_ids.to(model.device)
            attention_mask = torch.ones_like(input_ids).to(torch.bfloat16).to(model.device)
            ...
            if phase == "fullcache":
            ...
            elif phase == "actions" and portfolios is not None:
                outputs = [""] * len(portfolios)
                t0 = time.time()
                for pi, (ah, bh) in enumerate(portfolios):
                    cfg = _build_compression_config(
                        torch.tensor([ah[0]]), torch.tensor([bh[0]]), budget, ada_kv=ada_kv)
                    cfg.a_heads = ah; cfg.b_heads = bh
                    model.init_cache(cfg)
                    with torch.inference_mode():
                        out = model.generate(input_ids=input_ids, attention_mask=attention_mask,
                                             **gen_kwargs)
                    outputs[pi] = tokenizer.decode(out[0, ctx_len:], skip_special_tokens=True)
                ...
                for start in range(0, num_actions, action_batch_size):
                    ...
                    model.init_cache(_build_compression_config(a, b, budget, ada_kv=ada_kv, curve=curve))
                    with torch.inference_mode():
                        out = model.generate(
                            input_ids=in_ids,
                            attention_mask=attn,
...
def run_phase(
    ...
    for wid, g in enumerate(gpu_ids):
        p = ctx.Process(target=_worker, args=(wid, [g], model_name, tq, rq, budget, action_batch_size, ada_kv, actions, portfolios, curve))
        p.start()
        procs.append(p)
...
def main():
    ...
    ap.add_argument("--ada_kv", action="store_true",
                    help="Score the 13 actions WITH Ada-KV head-adaptive budgets "
                         "(train the routing on AdaKV-composed rewards).")
    ap.add_argument("--portfolios", type=str, default=None,
                    help="N3 head-portfolios: 'a:bxK+a:bxK,...' per config, e.g. "
                         "'10:16x4+0:1x4'. Scored sequentially (batch 1 each).")
    ap.add_argument("--curve", type=str, default="sigmoid",
                    choices=["sigmoid", "exp", "linear", "gauss"],
                    help="Weight-curve family (rebuttal alt-function study). Non-sigmoid "
                         "curves use b as the scale (tau/W/sigma); a is ignored.")
    ap.add_argument("--actions", type=str, default=None,
                    help="Override the action grid: 'a:b,a:b,...' (e.g. '1.0:1,1.0:16'). "
                         "Scores ONLY these actions (grid-extension scoring).")
    args = ap.parse_args()
    ...
    if args.portfolios:
        portfolios = []
        for spec in args.portfolios.split(","):
            ah, bh = [], []
            for part in spec.split("+"):
                ab, k = part.split("x")
                a, b = ab.split(":")
                ah += [float(a)] * int(k); bh += [float(b)] * int(k)
            portfolios.append((ah, bh))
        print(f"Portfolios: {portfolios}")
    custom_actions = None
    if args.actions:
        custom_actions = [(float(p.split(":")[0]), float(p.split(":")[1]))
                          for p in args.actions.split(",")]
        print(f"Custom action grid: {custom_actions}")
    act_results = run_phase("actions", missing_act, args.model, gpu_ids, args.budget,
                            args.action_batch_size, ada_kv=args.ada_kv, actions=custom_actions,
                            portfolios=portfolios, curve=args.curve)

    # compute scores per action, write to budget_out
    num_actions = (len(portfolios) if portfolios else
                   len(custom_actions) if custom_actions else len(SIGMOID_A_VALUES))
