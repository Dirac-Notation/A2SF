...
def make_layerwise_a2sf_mask(
    ...
    layer_scores = (a2sf_maps[:,:,:PROMPT_LENGTH,:] * forgetting).sum(dim=2, keepdim=True)
        ...
        if args.local_search:
            ...
            for layer_idx in tqdm(range(attention_maps.size(0))):
                ...
                for local_ratio_idx, local_ratio in enumerate(local_ratios):
                    ...
                    a2sf_results[local_ratio_idx] += F.cosine_similarity(original_output[layer_idx], condition_output.to("cuda:0"), dim=2).mean().item()
                
                max_idx = a2sf_results.index(max(a2sf_results))
                layerwise_local_ratio[layer_idx] = local_ratios[max_idx]
                
            del condition_maps, condition_output
            ...
            for layer_idx in tqdm(range(attention_maps.size(0))):
                ...
                for a2sf_factor_idx, a2sf_factor in enumerate(a2sf_factors):
                    ...
                    if FULL_SEARCH:
                        ...
                        condition_output = model.model.layers[layer_idx].post_attention_layernorm(condition_output)
                        condition_output = model.model.layers[layer_idx].mlp(condition_output)
                    a2sf_results[a2sf_factor_idx] += F.cosine_similarity(original_output[layer_idx], condition_output.to("cuda:0"), dim=2).mean().item()
