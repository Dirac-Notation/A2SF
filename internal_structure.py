...
for name, (i, j, k, h) in tqdm(methods.items()):
    ...
    for dataset, prompt in prompts.items():
        ...
        folder_path = os.path.join(root_path, "analysis", "hidden_state", "npy", dataset, name)
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        for i in range(len(result.hidden_states)):
            np.save(os.path.join(folder_path, f"{i}.npy"), result.hidden_states[i].cpu().detach().numpy())
            
        key_path = os.path.join(root_path, "analysis", "key_value", "npy", dataset, name, "key")
        if not os.path.exists(key_path):
            os.makedirs(key_path)

        value_path = os.path.join(root_path, "analysis", "key_value", "npy", dataset, name, "value")
        if not os.path.exists(value_path):
            os.makedirs(value_path)
            
        for i in range(len(result.past_key_values)):
            np.save(os.path.join(key_path, f"{i}.npy"), result.past_key_values[i][0].cpu().detach().numpy())
            np.save(os.path.join(value_path, f"{i}.npy"), result.past_key_values[i][1].cpu().detach().numpy())