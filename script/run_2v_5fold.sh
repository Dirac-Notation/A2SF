#!/usr/bin/env bash
# run_2v_5fold.sh <model> <lb_states> <mini_attn> <tag>
cd /home/smp9898/A2SF; PY=$HOME/miniconda3/envs/A2SF/bin/python
MODEL=$1; LBS=$2; MA=$3; TAG=$4
one(){ local fold=$1 gpu=$2; local pf=${TAG}f${fold}
  $PY script/cv_unified.py --model $MODEL --lb_states $LBS --out_prefix $pf --fold $fold --nfolds 5 >/dev/null 2>&1
  ...
  CUDA_VISIBLE_DEVICES=$gpu $PY RL/train.py --model $MODEL --budget 128 --states_file runs/states/${pf}_train.pt \
    ...
    --mini_attn_ckpt $MA --extra_view none --num_views 2 --loss listwise --loss_temp 0.1 \
  ...
  CUDA_VISIBLE_DEVICES=$gpu $PY script/fast_lb_eval.py --rl_checkpoint runs/${pf}_r/policy_best.pt --run_name $pf --budget 128 \
    --states_path runs/fast_lb_eval/${pf}_test_states.pt --index_path runs/fast_lb_eval/${pf}_test_index.pt 2>/dev/null | grep "Overall Average"|sed "s/^/${TAG} fold${fold}: /"
  rm -rf runs/${pf}_r runs/states/${pf}_train.pt datasets/cv/${pf}_* runs/fast_lb_eval/${pf}_*
}
